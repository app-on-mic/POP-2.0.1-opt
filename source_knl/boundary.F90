...
! !PUBLIC TYPES:
   ...
   type, public :: bndy
     integer (int_kind) :: &
       ...
       nlocal_nsneg_tiny   ! num local copies for east-west bndy update
     ...
     integer (int_kind), dimension(:), pointer :: &
       ...
       local_nsneg_src_block_tiny    ,&! source block for sending   n-s bndy msg
       local_nsneg_dst_block_tiny      ! dest   block for sending   n-s bndy msg
     ...
     integer (int_kind), dimension(:,:), pointer :: &
       ...
       local_nsneg_src_add_tiny      ,&! starting source address for n-s msgs
       local_nsneg_dst_add_tiny        ! starting dest   address for n-s msgs
   end type bndy
...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   interface
     ...
     subroutine print_trace() bind(C,name="print_trace")
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::           &
      ...
      iblk, imsg,                  &!
      iloc_ew, iloc_ns,            &!
      iloc_ew_tiny, iloc_ns_tiny,  &! temp counter for local pairs, ns handle positive dst_block only
      iloc_nsneg_tiny,            &! temp counter for negstive ns dst_block
   ...
   integer (int_kind), dimension(ny_bkfactor) :: &
      east_boulder_tiny_blocks, &
      west_boulder_tiny_blocks
   integer (int_kind), dimension(nx_bkfactor) :: &
...
!-----------------------------------------------------------------------
   ...
   if (lmic_proc) then
      ...
      newbndy%nlocal_nsneg_tiny = 2*nblocks_tot_tiny*(ny_bkfactor-1)/ny_bkfactor &
                               + newbndy%nlocal_ns*nx_bkfactor
      ...
      allocate (newbndy%local_ew_src_block_tiny(newbndy%nlocal_ew_tiny), &
                ...
                newbndy%local_nsneg_src_block_tiny(newbndy%nlocal_nsneg_tiny), &
                newbndy%local_nsneg_dst_block_tiny(newbndy%nlocal_nsneg_tiny), &
                ...
                newbndy%local_nsneg_src_add_tiny(2,newbndy%nlocal_nsneg_tiny), &
                newbndy%local_nsneg_dst_add_tiny(2,newbndy%nlocal_nsneg_tiny))
      ...
      newbndy%local_nsneg_src_block_tiny = 0
      newbndy%local_nsneg_dst_block_tiny = 0
      ...
      newbndy%local_nsneg_src_add_tiny = 0
      newbndy%local_nsneg_dst_add_tiny = 0
      iloc_ew_tiny = 0
      iloc_ns_tiny = 0
      iloc_nsneg_tiny = 0
         ...
         if (dst_proc /= 0) then
            ...
            if (iblock_dst == iblock_west .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  newbndy%local_ew_src_add(2,iloc_ew) = 1
                  !*** map to ie_src_tiny for MIC tiny-block process
                  if (lmic_proc) then
                  newbndy%local_ew_src_add(2,iloc_ew) = 1 + (src_block_loc-1)*nx_bkfactor*ny_bkfactor
                  endif
                  newbndy%local_ew_dst_block(iloc_ew) = dst_block_loc
            ...
            if ((iblock_dst == iblock_north .or. iblock_north < 0) .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  ! local copy for tiny block xiaobin
                  if (lmic_proc) then
                     if (iblock_north >= 0) then !*** not tripole boundary
                     iloc_ns_tiny = iloc_ns_tiny + nx_bkfactor
                     ...
                     !if (iblock_north < 0) then !*** tripole boundary
                        iloc_nsneg_tiny = iloc_nsneg_tiny + nx_bkfactor
                        newbndy%local_nsneg_src_block_tiny(iloc_nsneg_tiny-nx_bkfactor+1 : iloc_nsneg_tiny) = &
                                                              (src_block_loc-1)*nx_bkfactor*ny_bkfactor+north_boulder_tiny_blocks

                        newbndy%local_nsneg_dst_block_tiny(iloc_nsneg_tiny-nx_bkfactor+1 : iloc_nsneg_tiny) = &
                                                              -1*((dst_block_loc-1)*nx_bkfactor*ny_bkfactor+south_boulder_tiny_blocks)
                        !*** copy nghost+1 northern rows of physical
                        !*** domain into global north tripole buffer
                        iii=0
                        do ii = iloc_nsneg_tiny-nx_bkfactor+1 , iloc_nsneg_tiny
                           newbndy%local_nsneg_src_add_tiny(1, ii) = &
                                              src_block%i_glob(nghost+1)+iii*block_size_x
                           newbndy%local_nsneg_src_add_tiny(2, ii) = &
                                              block_size_y
                          ! newbndy%local_nsneg_src_add(2, ii) = &
                          !                    dst_block%je - nghost
                           !*** copy out of tripole ghost cell buffer
                           !*** over-write the last row of the destination
                           !*** block to enforce for symmetry for fields
                           !*** located on domain boundary
                           newbndy%local_nsneg_dst_add_tiny(1, ii) = &
                                                dst_block%i_glob(nghost+1)+iii*block_size_x
                           newbndy%local_nsneg_dst_add_tiny(2, ii) = &
                  ...
                  !write(6,"(A,12I5)")'xiaobin createbound1.21,task,iloc_ns,dst_block,dst_block_loc-1',my_task,iloc_ns_tiny,newbndy%local_ns_dst_block_tiny(iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny),dst_block_loc-1,nx_bkfactor*ny_bkfactor,south_boulder_tiny_blocks(:)
                     ...
                     !if (iblock_north < 0) then !*** tripole boundary
                        iloc_nsneg_tiny = iloc_nsneg_tiny + nx_bkfactor
                        newbndy%local_nsneg_src_block_tiny(iloc_nsneg_tiny-nx_bkfactor+1 : iloc_nsneg_tiny) = 0
                        newbndy%local_nsneg_src_add_tiny(1,iloc_nsneg_tiny-nx_bkfactor+1 : iloc_nsneg_tiny) = 0
                        newbndy%local_nsneg_src_add_tiny(2,iloc_nsneg_tiny-nx_bkfactor+1 : iloc_nsneg_tiny) = 0
                        ...
                        do ii = iloc_nsneg_tiny-nx_bkfactor+1 , iloc_nsneg_tiny
                           newbndy%local_nsneg_dst_add_tiny(1,ii) = &
                                                   dst_block%i_glob(nghost+1)+iii*block_size_x
                           newbndy%local_nsneg_dst_add_tiny(2,ii) = block_size_y+nghost
                           !newbndy%local_nsneg_dst_add_tiny(2,ii) = dst_block%je
                           iii = iii+1
                        end do
                     endif
                  end if
                  ...
                  if (lmic_proc) then
                  newbndy%ns_src_add(1,iblk,imsg) = (ny_bkfactor-1)*nx_bkfactor + (src_block_loc-1) * nx_bkfactor *ny_bkfactor
                  newbndy%ns_src_add(2,iblk,imsg) = src_block%je - nghost + 1 &
                                                   - (ny_bkfactor-1)*block_size_y
                  endif
            ...
            if (iblock_dst == iblock_south .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  !*** map to ie_dst_tiny for MIC tiny-block process
                  if (lmic_proc) then
                  newbndy%ns_src_add(1,iblk,imsg) = (src_block_loc-1) * nx_bkfactor *ny_bkfactor
                  endif
                  newbndy%nblocks_ns_snd(imsg) = &
                  newbndy%nblocks_ns_snd(imsg) + 1
               else if (dst_proc == my_task+1 .and. &
...
!-----------------------------------------------------------------------
   ...
   if (lmic_proc) then
   if (iloc_ns_tiny /= newbndy%nlocal_ns_tiny) &
      write(6,*) 'local_ns_tiny: ',my_task, iloc_ns_tiny, newbndy%nlocal_ns_tiny
   newbndy%nlocal_ns_tiny = iloc_ns_tiny
   newbndy%nlocal_nsneg_tiny = iloc_nsneg_tiny
   endif
   ...
   if (lmic_proc) then
      ...
      do k=1, nblocks_tot_tiny !number of internal tiny block boundarys
         if (mod(k, nx_bkfactor) /= 0) then
            ...
            ! src is dst, dst is scr
            newbndy%local_ew_src_block_tiny(iloc_ew_tiny) = k+1
            newbndy%local_ew_src_add_tiny(1,iloc_ew_tiny) = cacg_matlevel + 1
            newbndy%local_ew_src_add_tiny(2,iloc_ew_tiny) = 1
            newbndy%local_ew_dst_block_tiny(iloc_ew_tiny) = k
            newbndy%local_ew_dst_add_tiny(1,iloc_ew_tiny) = cacg_matlevel + block_size_x + 1
            newbndy%local_ew_dst_add_tiny(2,iloc_ew_tiny) = 1
         end if
         if (mod(k, nx_bkfactor*ny_bkfactor) <= nx_bkfactor*(ny_bkfactor-1) .and.&
             mod(k, nx_bkfactor*ny_bkfactor) /= 0) then
            ...
            newbndy%local_ns_src_block_tiny(iloc_ns_tiny) = k + nx_bkfactor
            newbndy%local_ns_src_add_tiny(1,iloc_ns_tiny) = 1
            newbndy%local_ns_src_add_tiny(2,iloc_ns_tiny) = cacg_matlevel + 1
            newbndy%local_ns_dst_block_tiny(iloc_ns_tiny) = k
            newbndy%local_ns_dst_add_tiny(1,iloc_ns_tiny) = 1
            newbndy%local_ns_dst_add_tiny(2,iloc_ns_tiny) = cacg_matlevel + block_size_y + 1
         endif
         !write(6,*)'my_task+1,iloc_ew_tiny,iloc_ns_tiny',my_task+1,iloc_ew_tiny,iloc_ns_tiny
      enddo
      !call MPI_BARRIER(MPI_COMM_OCN, ierr)
   ...
   block_loop2: do n=1,nblocks
      ...
      if (src_block_loc /= 0 .and. src_proc == my_task+1) then
         if (iblock_east == 0) then
            ...
            newbndy%local_ew_src_add(2,iloc_ew) = 0
            newbndy%local_ew_dst_block(iloc_ew) = src_block_loc
            newbndy%local_ew_dst_add(1,iloc_ew) = src_block%ie + (cacg_matlevel-nghost) + 1
            newbndy%local_ew_dst_add(2,iloc_ew) = 1
            !*** map to ie_dst_tiny for MIC tiny-block process
            if (lmic_proc) then
               iloc_ew_tiny = iloc_ew_tiny + ny_bkfactor
               ...
               newbndy%local_ew_dst_block_tiny(iloc_ew_tiny-ny_bkfactor+1 : iloc_ew_tiny) = &
                                                               (src_block_loc-1)*nx_bkfactor*ny_bkfactor+east_boulder_tiny_blocks
               newbndy%local_ew_dst_add_tiny(1,iloc_ew_tiny-ny_bkfactor+1 : iloc_ew_tiny) = cacg_matlevel + block_size_x + 1
               newbndy%local_ew_dst_add_tiny(2,iloc_ew_tiny-ny_bkfactor+1 : iloc_ew_tiny) = 1
            end if
         else if (iblock_west == 0) then
            iloc_ew = iloc_ew + 1
            ...
            newbndy%local_ns_dst_block(iloc_ns) = src_block_loc
            newbndy%local_ns_dst_add(1,iloc_ns) = 1
            newbndy%local_ns_dst_add(2,iloc_ns) = src_block%je + (cacg_matlevel-nghost) + 1
            if (lmic_proc) then
               iloc_ns_tiny = iloc_ns_tiny + nx_bkfactor
               newbndy%local_ns_src_block_tiny(iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = 0
               newbndy%local_ns_src_add_tiny(1,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = 0
               ...
               newbndy%local_ns_dst_block_tiny(iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = &
                                                              (src_block_loc-1)*nx_bkfactor*ny_bkfactor+north_boulder_tiny_blocks
               newbndy%local_ns_dst_add_tiny(1,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = 1
               newbndy%local_ns_dst_add_tiny(2,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = cacg_matlevel + block_size_y + 1
            end if
         else if (jblock_south == 0) then
            iloc_ns = iloc_ns + 1
            newbndy%local_ns_src_block(iloc_ns) = 0
         ...
         if (dst_proc /= 0) then
            ...
            if (iblock_dst == iblock_west .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  newbndy%local_ew_dst_block(iloc_ew) = dst_block_loc
                  newbndy%local_ew_dst_add(1,iloc_ew) = dst_block%ie + (cacg_matlevel-nghost) + 1
                  newbndy%local_ew_dst_add(2,iloc_ew) = 1
                  ...
                  newbndy%ew_rcv_proc (     imsg) = src_proc
                  newbndy%ew_dst_block(iblk,imsg) = dst_block_loc
                  newbndy%ew_dst_add(1,iblk,imsg) = dst_block%ie +(cacg_matlevel-nghost) + 1
                  newbndy%ew_dst_add(2,iblk,imsg) = 1
                  newbndy%nblocks_ew_rcv(imsg) = &
                  newbndy%nblocks_ew_rcv(imsg) + 1
               endif
            ...
            if ((iblock_dst == iblock_north .or. iblock_north < 0) .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  if (iblock_north < 0) then !*** tripole boundary
                     ...
                     newbndy%local_ns_dst_add(2,iloc_ns) = & 
                                          dst_block%je + (cacg_matlevel-nghost)
                  endif
                  ...
                  if (lmic_proc) then
                     ...
                     !if (iblock_north < 0) then !*** tripole boundary
                        ...
                        do ii = iloc_nsneg_tiny-nx_bkfactor+1 , iloc_nsneg_tiny
                           newbndy%local_nsneg_src_add_tiny(1, ii) = &
                                              src_block%i_glob(cacg_matlevel+1)+iii*block_size_x
                           newbndy%local_nsneg_src_add_tiny(2, ii) = &
                                              block_size_y
                          ! newbndy%local_nsneg_src_add(2, ii) = &
                          !                    dst_block%je - nghost
                           ...
                           !*** located on domain boundary
                           newbndy%local_nsneg_dst_add_tiny(1, ii) = &
                                                dst_block%i_glob(cacg_matlevel+1)+iii*block_size_x
                           newbndy%local_nsneg_dst_add_tiny(2, ii) = &
                                                block_size_y+2*cacg_matlevel
                           iii = iii+1
                        enddo
                     endif
                  endif
                  ...
                  if (iblock_north < 0) then !*** tripole boundary
                     ...
                     newbndy%local_ns_dst_add(1,iloc_ns) = &
                                             dst_block%i_glob(cacg_matlevel+1)
                     newbndy%local_ns_dst_add(2,iloc_ns) = dst_block%je + (cacg_matlevel-nghost)
                  endif
                  ...
                  !write(6,"(A,12I5)")'xiaobin createbound1.21,task,iloc_ns,dst_block,dst_block_loc-1',my_task,iloc_ns_tiny,newbndy%local_ns_dst_block_tiny(iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny),dst_block_loc-1,nx_bkfactor*ny_bkfactor,south_boulder_tiny_blocks(:)
                     ...
                     !if (iblock_north < 0) then !*** tripole boundary
                        ...
                        do ii = iloc_nsneg_tiny-nx_bkfactor+1 , iloc_nsneg_tiny
                           newbndy%local_nsneg_dst_add_tiny(1,ii) = &
                                                   dst_block%i_glob(cacg_matlevel+1)+iii*block_size_x
                           newbndy%local_nsneg_dst_add_tiny(2,ii) = block_size_y+cacg_matlevel
                           !newbndy%local_nsneg_dst_add_tiny(2,ii) = dst_block%je
                           iii = iii+1
                        end do
                     endif
                  ...
                  if (lmic_proc) then
                  newbndy%ns_src_add(1,iblk,imsg) = (ny_bkfactor-1)*nx_bkfactor + (src_block_loc-1) * nx_bkfactor *ny_bkfactor
                  newbndy%ns_src_add(2,iblk,imsg) = block_size_y + 1
                  endif
                  newbndy%nblocks_ns_snd(imsg) = &
                  newbndy%nblocks_ns_snd(imsg) + 1
                  if (iblock_north < 0) then !*** tripole boundary
                     ...
                     !*** map to ie_dst_tiny for MIC tiny-block process
                     if (lmic_proc) &
                     newbndy%ns_src_add(2,iblk,imsg) = block_size_y
                  endif
               else if (dst_proc == my_task+1 .and. &
                        src_proc /= my_task+1) then
                  !*** message must be received
                  ...
                  if (iblock_north < 0) then !*** tripole
                     ...
                     newbndy%ns_dst_add(1,iblk,imsg) = &
                                            src_block%i_glob(cacg_matlevel+1)
                     !*** addresses for copying out of ghost buffer
                     newbndy%ns_dst_add(2,iblk,imsg) = dst_block%je + (cacg_matlevel-nghost)
                     newbndy%ns_dst_add(3,iblk,imsg) = &
                                       dst_block%i_glob(cacg_matlevel+1)
                  endif
               endif
            ...
            if (iblock_dst == iblock_south .and. &
               ...
               if (src_proc == my_task+1 .and. &
                  ...
                  newbndy%local_ns_dst_block(iloc_ns) = dst_block_loc
                  newbndy%local_ns_dst_add(1,iloc_ns) = 1
                  newbndy%local_ns_dst_add(2,iloc_ns) = dst_block%je +(cacg_matlevel-nghost) + 1
                  ! local copy for tiny block xiaobin
                  if (lmic_proc) then
                     iloc_ns_tiny = iloc_ns_tiny + nx_bkfactor
                     newbndy%local_ns_src_block_tiny(iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = &
                                                              (src_block_loc-1)*nx_bkfactor*ny_bkfactor+south_boulder_tiny_blocks
                     newbndy%local_ns_src_add_tiny(1,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = 1
                     newbndy%local_ns_src_add_tiny(2,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = cacg_matlevel + 1

                     newbndy%local_ns_dst_block_tiny(iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = &
                                                              (dst_block_loc-1)*nx_bkfactor*ny_bkfactor+north_boulder_tiny_blocks
                     newbndy%local_ns_dst_add_tiny(1,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = 1
                     newbndy%local_ns_dst_add_tiny(2,iloc_ns_tiny-nx_bkfactor+1 : iloc_ns_tiny) = &
                                                              cacg_matlevel + block_size_y + 1
                  ...
                  newbndy%ns_dst_block(iblk,imsg) = dst_block_loc
                  newbndy%ns_dst_add(1,iblk,imsg) = 1
                  newbndy%ns_dst_add(2,iblk,imsg) = dst_block%je + (cacg_matlevel-nghost) + 1
                  newbndy%nblocks_ns_rcv(imsg) = &
                  newbndy%nblocks_ns_rcv(imsg) + 1
               endif
            endif ! south neighbor
...
!-----------------------------------------------------------------------
   ...
   allocate(buf_ew_snd(nghost, block_size_y, &
                       in_bndy%maxblocks_ew_snd, in_bndy%nmsg_ew_snd),&
            buf_ew_rcv(nghost, block_size_y, &
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_rcv
      ...
      bufsize = block_size_y*nghost*in_bndy%nblocks_ew_rcv(n)
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_snd
      ...
      bufsize = block_size_y*nghost*in_bndy%nblocks_ew_snd(n)

      do i=1,in_bndy%nblocks_ew_snd(n)
         ib_src    = in_bndy%ew_src_add(1,i,n)
         ie_src    = ib_src + nghost - 1
         src_block = in_bndy%ew_src_block(i,n)
         buf_ew_snd(:,:,i,n) = ARRAY(ib_src:ie_src,nghost+1:nghost+block_size_y,src_block)
      end do
...
!-----------------------------------------------------------------------
   ...
   do k=1,in_bndy%nblocks_ew_rcv(n)
      ...
      ARRAY(ib_dst:ie_dst,nghost+1:nghost+block_size_y,dst_block) = buf_ew_rcv(:,:,k,n)
   end do
   end do
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::           &
      ...
      isign,                       &! sign factor for tripole grids
      ierr,                        &! MPI error flag
      tid                           ! thread id in OpenMP region
...
!      ! east-west
!      !$OMP PARALLEL DO COLLAPSE(2) PRIVATE(jblock,iblock,n,j,i)
...
!-----------------------------------------------------------------------
   ...
   allocate(buf_ew_snd(nghost, block_size_y_mac, &
                       in_bndy%maxblocks_ew_snd, in_bndy%nmsg_ew_snd),&
            buf_ew_rcv(nghost, block_size_y_mac, &
                       in_bndy%maxblocks_ew_rcv, in_bndy%nmsg_ew_rcv))
   ...
   !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(i,j,k,m,n,ib_src,ie_src,jb_src,je_src,ib_dst,ie_dst,jb_dst,je_dst,ib_src_tiny,ie_src_tiny,jb_src_tiny,je_src_tiny,ib_dst_tiny,ie_dst_tiny,jb_dst_tiny,je_dst_tiny,iblock,jblock,iblock2,jblock2,src_tiny_block,dst_tiny_block,src_block,dst_block,bufsize,ierr,tid)
#ifdef _OPENMP
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_rcv
      ...
      bufsize = block_size_y_mac*nghost*in_bndy%nblocks_ew_rcv(n)
   ...
   !call timer_stop(bndy_2d_recv)
   !write(6,*) 'section1_s_done: ',my_task,tid

   endif
   !write(6,*) 'section1_p_begin: ',my_task,tid
!-----------------------------------------------------------------------
!
!  fill send buffer and post sends
!
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_send)
   !write(6,*) 'bound_dbl_tiny:EW_SND:n',my_task,in_bndy%nmsg_ew_snd
   !$OMP DO COLLAPSE(2) PRIVATE(n,i,ib_src,src_block,iblock,jblock,&
   !$OMP                     ib_src_tiny,ie_src_tiny,src_tiny_block)
   do n=1,in_bndy%nmsg_ew_snd
      do jblock=1,ny_bkfactor
      do i=1,in_bndy%nblocks_ew_snd(n)
         ib_src    = in_bndy%ew_src_add(1,i,n)
         ...
         ie_src_tiny = ib_src_tiny + nghost - 1
            ...
            buf_ew_snd(:,(jblock-1)*block_size_y+1:jblock*block_size_y,i,n) = &
   ...
   !$OMP END DO
   !write(6,*) 'section1_p_done: ',my_task,tid

   if (tid == 0) then
   do n=1,in_bndy%nmsg_ew_snd
      bufsize = block_size_y_mac*nghost*in_bndy%nblocks_ew_snd(n)
      call MPI_ISEND(buf_ew_snd(1,1,1,n), bufsize, mpi_dbl, &
                     in_bndy%ew_snd_proc(n)-1, &
                     mpitag_bndy_2d + my_task + 1, &
                     in_bndy%communicator, snd_request(n), ierr)
...
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_local)
   !write(6,*) 'bound_dbl_tiny:EW_SND:n',my_task,in_bndy%nmsg_ew_snd
   !$OMP DO SIMD PRIVATE(src_block,dst_block,ib_src,ib_dst,ie_src,ie_dst)
...
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_final)
   !$OMP DO COLLAPSE(2) PRIVATE(n,k,iblock,jblock,dst_block,ib_dst,ie_dst,&
   !$OMP                     ib_src_tiny,ie_src_tiny,dst_tiny_block)
   do n=1,in_bndy%nmsg_ew_rcv
   do jblock=1,ny_bkfactor
     do k=1,in_bndy%nblocks_ew_rcv(n)
      dst_block = in_bndy%ew_dst_block(k,n)
      ...
      ie_dst_tiny = ib_dst_tiny + nghost - 1
         ...
         ARRAY(ib_dst_tiny:ie_dst_tiny,nghost+1:nghost+block_size_y,dst_tiny_block) = &
            buf_ew_rcv(1:nghost,(jblock-1)*block_size_y+1:jblock*block_size_y,k,n)
     end do
   end do
   end do
   !$OMP END DO
...
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_send)
   !$OMP DO COLLAPSE(2) PRIVATE(n,i,iblock,jblock,&
   !$OMP                     jb_src_tiny,je_src_tiny,src_tiny_block)
   do n=1,in_bndy%nmsg_ns_snd
      do iblock=1,nx_bkfactor

      do i=1,in_bndy%nblocks_ns_snd(n)
         jb_src_tiny = in_bndy%ns_src_add(2,i,n)
         je_src_tiny = jb_src_tiny + nghost  ! nghost+1 rows needed for tripole
         !src_block = in_bndy%ns_src_block(i,n)
         !buf_ns_snd(:,:,i,n) = ARRAY(:,jb_src:je_src,src_block)
         jblock = in_bndy%ns_src_add(1,i,n)
         !jblock=(jb_src-nghost-2)/block_size_y+1
         if (iblock == 1) then
         buf_ns_snd(1:nghost,:,i,n) = ARRAY(1:nghost,jb_src_tiny:je_src_tiny,&
                         1 + jblock)
         endif

         src_tiny_block = iblock + jblock
         ...
         if (iblock == nx_bkfactor) then
         buf_ns_snd(nghost+(nx_bkfactor)*block_size_x+1:nx_block_mac,:,i,n) = &
            ARRAY(nghost+block_size_x+1:nx_block,jb_src_tiny:je_src_tiny,nx_bkfactor + jblock)
         endif
      end do
      end do
   end do
...
!-----------------------------------------------------------------------
   ...
   !$OMP DO PRIVATE(src_block,dst_block,ib_src,ib_dst,ie_src,ie_dst,jb_src,jb_dst,je_src,je_dst)
   ...
   !$OMP END DO
   !$OMP DO PRIVATE(n,src_block,dst_block,ib_src,ib_dst,ie_src,ie_dst,jb_src,jb_dst,je_src,je_dst)
   do n=1,in_bndy%nlocal_nsneg_tiny
      src_block = in_bndy%local_nsneg_src_block_tiny(n)
      dst_block = in_bndy%local_nsneg_dst_block_tiny(n)
      !if (dst_block > 0) then ! straight local copy
      !else  !north boundary tripole grid - copy into global north buffer
         jb_src = in_bndy%local_nsneg_src_add_tiny(2,n)
         je_src = jb_src + nghost ! need nghost+1 rows for tripole

         !*** determine start, end addresses of physical domain
         !*** for both global buffer and local block

         ib_dst = in_bndy%local_nsneg_src_add_tiny(1,n)
...
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_final)
   !$OMP DO PRIVATE(n,k,iblock,jblock,dst_block,ib_src,ie_src,ib_dst,ie_dst,jb_dst,je_dst,&
   !$OMP                     jb_dst_tiny,je_dst_tiny,dst_tiny_block)
   do n=1,in_bndy%nmsg_ns_rcv
   do k=1,in_bndy%nblocks_ns_rcv(n)
      !??? src_block = in_bndy%local_ns_src_block(n)
      dst_block = in_bndy%ns_dst_block(k,n)  ! dest block
      ...
      if (dst_block > 0) then  ! normal receive
         ...
         ib_dst = in_bndy%ns_dst_add(1,k,n)
         ie_dst = ib_dst + block_size_x_mac - 1
         if (ie_dst > nx_global) ie_dst = nx_global
         ib_src = nghost + 1
         ie_src = ib_src + ie_dst - ib_dst
         if (src_block /= 0) then
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ns_snd
      ...
      do i=1,in_bndy%nblocks_ns_snd(n)
         jb_src_tiny    = in_bndy%ns_src_add(2,i,n)
         je_src_tiny    = jb_src + nghost  ! nghost+1 rows needed for tripole
         !src_block = in_bndy%ns_src_block(i,n)
         !buf_ns_snd(:,:,i,n) = ARRAY(:,jb_src:je_src,src_block)
         jblock = in_bndy%ns_src_add(1,i,n)
         src_tiny_block = 1 + jblock
         ...
         src_tiny_block = nx_bkfactor + jblock
...
!-----------------------------------------------------------------------
   ...
   deallocate(snd_request, rcv_request, snd_status, rcv_status)
      ...
      call matpow_halo_2d_dbl_tiny(ARRAY, in_bndy, grid_loc, field_type)
   endif  ! lmic_proc
!-----------------------------------------------------------------------
...
! !INTERFACE:
 ...
 subroutine matpow_halo_2d_dbl_tiny(ARRAY, in_bndy, grid_loc, field_type)
...
!-----------------------------------------------------------------------
   ...
   allocate(buf_ew_snd(cacg_matlevel, block_size_y_mac, &
                       in_bndy%maxblocks_ew_snd, in_bndy%nmsg_ew_snd),&
            buf_ew_rcv(cacg_matlevel, block_size_y_mac, &
                       in_bndy%maxblocks_ew_rcv, in_bndy%nmsg_ew_rcv))
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_rcv
      ...
      bufsize = block_size_y_mac*cacg_matlevel*in_bndy%nblocks_ew_rcv(n)
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ew_snd
      ...
      do i=1,in_bndy%nblocks_ew_snd(n)
         ...
         src_block = in_bndy%ew_src_block(i,n) ! macro-block
         !buf_ew_snd(:,:,i,n) = ARRAY(ib_src:ie_src,:,src_block)
         if (ib_src <= 2*cacg_matlevel) then
            iblock = 1
         else
            iblock = nx_bkfactor
         endif
         !iblock=(ib_src-cacg_matlevel-2)/block_size_x+1
         ib_src_tiny = ib_src-(iblock-1)*block_size_x
         ie_src_tiny = ib_src_tiny + cacg_matlevel - 1

            src_tiny_block = iblock + (jblock-1)*nx_bkfactor+(src_block-1)*nx_bkfactor*ny_bkfactor
            buf_ew_snd(:,(jblock-1)*block_size_y+1:jblock*block_size_y,i,n) = &
               ARRAY(ib_src_tiny:ie_src_tiny,cacg_matlevel+1:cacg_matlevel+block_size_y,src_tiny_block)
   ...
   do n=1,in_bndy%nmsg_ew_snd
      bufsize = block_size_y_mac*cacg_matlevel*in_bndy%nblocks_ew_snd(n)
      call MPI_ISEND(buf_ew_snd(1,1,1,n), bufsize, mpi_dbl, &
                     in_bndy%ew_snd_proc(n)-1, &
                     mpitag_bndy_2d + my_task + 1, &
                     in_bndy%communicator, snd_request(n), ierr)
...
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_local)
   !write(6,*) 'bound_dbl_tiny:EW_SND:n',my_task,in_bndy%nlocal_ew_tiny
   !$OMP DO SIMD PRIVATE(src_block,dst_block,ib_src,ib_dst,ie_src,ie_dst)
   do n=1,in_bndy%nlocal_ew_tiny
      src_block = in_bndy%local_ew_src_block_tiny(n)
      dst_block = in_bndy%local_ew_dst_block_tiny(n)
      ...
      ib_dst = in_bndy%local_ew_dst_add_tiny(1,n)
      ie_dst = ib_dst + cacg_matlevel - 1
      !write(6,"(A,I3,2(A,I3,2I4))")'local_ew_tiny:',my_task,' src(:):',src_block,ib_src,ie_src,'dst(:)',dst_block,ib_dst,ie_dst
   ...
   !$OMP END DO SIMD
!   if (my_task == nproc_cpu_pn*nnode) &
!      write(6,*) 'bound_2d_dbl_tiny:update_local',my_task,sum(ARRAY(cacg_matlevel+1:nx_block-cacg_matlevel,cacg_matlevel+1:ny_block-cacg_matlevel,1:nx_bkfactor*ny_bkfactor))
   !call timer_stop(bndy_2d_local)
...
!-----------------------------------------------------------------------
   ...
   do jblock=1,ny_bkfactor
     do k=1,in_bndy%nblocks_ew_rcv(n)
      ...
      !ARRAY(ib_dst:ie_dst,:,dst_block) = buf_ew_rcv(:,:,k,n)
      iblock=(ib_dst-cacg_matlevel-2)/block_size_x+1
      ib_dst_tiny = ib_dst-(iblock-1)*block_size_x
      ie_dst_tiny = ib_dst_tiny + cacg_matlevel - 1

      dst_tiny_block = iblock + (jblock-1)*nx_bkfactor+(dst_block-1)*nx_bkfactor*ny_bkfactor
      ARRAY(ib_dst_tiny:ie_dst_tiny,1+cacg_matlevel:1 * cacg_matlevel + block_size_y, dst_tiny_block) = &
            buf_ew_rcv(1:cacg_matlevel,(jblock-1)*block_size_y+1:jblock * block_size_y,k,n)
     end do
   end do
   end do
   !$OMP END DO
...
!-----------------------------------------------------------------------
   ...
   allocate(buf_ns_snd((block_size_x_mac + 2 * cacg_matlevel), cacg_matlevel+1, &
                       in_bndy%maxblocks_ns_snd, in_bndy%nmsg_ns_snd),&
            buf_ns_rcv((block_size_x_mac + 2 * cacg_matlevel), cacg_matlevel+1, &
                       in_bndy%maxblocks_ns_rcv, in_bndy%nmsg_ns_rcv))
...
!-----------------------------------------------------------------------
   ...
   do n=1,in_bndy%nmsg_ns_rcv
      ...
      bufsize = (block_size_x_mac + 2 * cacg_matlevel)*(cacg_matlevel+1)*in_bndy%nblocks_ns_rcv(n)
...
!-----------------------------------------------------------------------
   ...
   !call timer_start(bndy_2d_send)
   !$OMP DO COLLAPSE(2) PRIVATE(n,i,iblock,jblock,jb_src_tiny,je_src_tiny,src_tiny_block,&
   !$OMP                     ib_dst,ie_dst,ib_src,ie_src)
   do n=1,in_bndy%nmsg_ns_snd
      do iblock=1,nx_bkfactor

      do i=1,in_bndy%nblocks_ns_snd(n)
         jb_src_tiny = in_bndy%ns_src_add(2,i,n)
         je_src_tiny = jb_src_tiny + cacg_matlevel  ! cacg_matlevel+1 rows needed for tripole
         !src_block = in_bndy%ns_src_block(i,n)
         !buf_ns_snd(:,:,i,n) = ARRAY(:,jb_src:je_src,src_block)
         jblock = in_bndy%ns_src_add(1,i,n)
         !jblock=(jb_src-cacg_matlevel-2)/block_size_y+1
         src_tiny_block = iblock + jblock
         ib_dst = cacg_matlevel+(iblock-1)*block_size_x+1
         ie_dst = cacg_matlevel+iblock*block_size_x
         ib_src = cacg_matlevel+1
         if (iblock == 1) then
            ib_dst = ib_dst - cacg_matlevel
            ib_src = ib_src - cacg_matlevel
         endif
         if (iblock == nx_bkfactor) then
            ie_dst = ie_dst + cacg_matlevel
            ie_src = ie_src + cacg_matlevel
         endif

         buf_ns_snd(ib_dst:ie_dst,:,i,n) = &
            ARRAY(ib_src:ie_src,jb_src_tiny:je_src_tiny,src_tiny_block)
      end do
      end do
   end do
   !$OMP END DO
   ...
   if (tid == 0) then
   do n=1,in_bndy%nmsg_ns_snd
      bufsize = (block_size_x_mac + 2 * cacg_matlevel)*(cacg_matlevel+1)*in_bndy%nblocks_ns_snd(n)
      call MPI_ISEND(buf_ns_snd(1,1,1,n), bufsize, mpi_dbl, &
                     in_bndy%ns_snd_proc(n)-1, &
                     mpitag_bndy_2d + my_task + 1, &
                     in_bndy%communicator, snd_request(n), ierr)
...
!-----------------------------------------------------------------------
   ...
   do k=1,in_bndy%nblocks_ns_rcv(n)
      ...
      if (dst_block > 0) then  ! normal receive
         ...
         !ARRAY(:,jb_dst:je_dst,dst_block) = buf_ns_rcv(:,1:cacg_matlevel,k,n)
         jblock=(jb_dst-cacg_matlevel-2)/block_size_y+1
         jb_dst_tiny = jb_dst-(jblock-1)*block_size_y
         je_dst_tiny = jb_dst_tiny + cacg_matlevel - 1
   
         do iblock=1,nx_bkfactor
            dst_tiny_block = iblock + (jblock-1)*nx_bkfactor+(dst_block-1)*nx_bkfactor*ny_bkfactor
            ARRAY(1:2 * cacg_matlevel + block_size_x, jb_dst_tiny:je_dst_tiny,dst_tiny_block) = &
               buf_ns_rcv((iblock-1)*block_size_x+1:2*cacg_matlevel+iblock*block_size_x,1:cacg_matlevel,k,n)
         end do
      else ! northern tripole bndy: copy into global tripole buffer
      ...
      do n=1,in_bndy%nlocal_ns
         ...
         if (dst_block < 0) then
            ...
            !*** east-west ghost cell info in the north boundary
               jblock=(jb_dst-cacg_matlevel-2)/block_size_y+1
               jb_dst_tiny = jb_dst-(jblock-1)*block_size_y
               je_dst_tiny = jb_dst_tiny + cacg_matlevel
            ...
            if (ib_src == 1) then  ! western boundary
               ...
               !*** of domain from tripole ghost cell buffer
               dst_tiny_block = 1 + (jblock-1)*nx_bkfactor+ (dst_block-1)*nx_bkfactor*ny_bkfactor
               ARRAY(1:cacg_matlevel,jb_dst_tiny:je_dst_tiny,dst_tiny_block) = &
                  tripole_dghost_capcg(nx_global-cacg_matlevel+1:nx_global,:)
               ie_src = ib_src + (block_size_x_mac + 2 * cacg_matlevel) - cacg_matlevel - 1
               if (ie_src > nx_global) ie_src = nx_global
               ib_dst = cacg_matlevel + 1
               ie_dst = ib_dst + (ie_src - ib_src)
            else
               ib_src = ib_src - cacg_matlevel
               ie_src = ib_src + (block_size_x_mac + 2 * cacg_matlevel) - 1
               if (ie_src > nx_global) ie_src = nx_global
               ib_dst = 1
               ie_dst = ib_dst + (ie_src - ib_src)
            endif
            if (ie_src == nx_global) then ! eastern boundary
               !*** impose cyclic conditions in ghost cells
               iblock=(ie_dst-cacg_matlevel-2)/block_size_x+1
               ie_dst_tiny = ie_dst-(iblock-1)*block_size_x
               dst_tiny_block = iblock + (jblock-1)*nx_bkfactor+ (dst_block-1)*nx_bkfactor*ny_bkfactor
               ARRAY(ie_dst_tiny+1:ie_dst_tiny+cacg_matlevel,jb_dst_tiny:je_dst_tiny,dst_tiny_block) = &
                  tripole_dghost_capcg(1:cacg_matlevel,:)
            endif
            ...
            iblock=(ib_dst-cacg_matlevel-2)/block_size_x+1
            ib_dst_tiny = ib_dst-(iblock-1)*block_size_x
            iblock2=(ie_dst-cacg_matlevel-2)/block_size_x+1
            ie_dst_tiny = ie_dst-(iblock2-1)*block_size_x
            do i=iblock,iblock2-1
               dst_tiny_block = i + (jblock-1)*nx_bkfactor+ (src_block-1)*nx_bkfactor*ny_bkfactor
               ARRAY(ib_dst_tiny:2*cacg_matlevel+block_size_x,jb_dst_tiny:je_dst_tiny,dst_tiny_block) = &
                  tripole_dghost_capcg(ib_src:ib_src+(2*cacg_matlevel+block_size_x-ib_dst_tiny),:)
               ib_src = ib_src + (block_size_x-ib_dst_tiny+1)
               ib_dst_tiny = 1
            end do
            dst_tiny_block = iblock2 + (jblock-1)*nx_bkfactor+ (src_block-1)*nx_bkfactor*ny_bkfactor
      ...
      do k=1,in_bndy%nblocks_ns_rcv(n)
         ...
         if (dst_block < 0) then
            ...
            if (ib_src == 1) then  ! western boundary
               ...
               ARRAY(1:cacg_matlevel,jb_dst_tiny:je_dst_tiny,dst_tiny_block) = &
                  tripole_dghost_capcg(nx_global-cacg_matlevel+1:nx_global,:)
               ie_src = ib_src + 2*cacg_matlevel+block_size_x_mac - cacg_matlevel - 1
               if (ie_src > nx_global) ie_src = nx_global
               ib_dst = cacg_matlevel + 1
               ie_dst = ib_dst + (ie_src - ib_src)
            else
               ib_src = ib_src - cacg_matlevel
               ie_src = ib_src + 2*cacg_matlevel+block_size_x_mac - 1
               if (ie_src > nx_global) ie_src = nx_global
               ib_dst = 1
               ie_dst = ib_dst + (ie_src - ib_src)
            endif
...
!-----------------------------------------------------------------------
 ...
 end subroutine matpow_halo_2d_dbl_tiny
