...
!   where (k > KMT(:,:,bid)) DIV_OUT = c0
    call div_1d(k,DIV_OUT,UX,UY,DXU(:,:,bid),DYU(:,:,bid),KMT(:,:,bid))
...
!DIR$ ATTRIBUTES FORCEINLINE :: div_1d
 subroutine div_1d(k,DIV_OUT,UX,UY,DXU,DYU,KMT)
...
! !INPUT PARAMETERS
   ...
   real (r8), dimension(1:nx_block*ny_block), intent(in) :: & 
      UX,UY,             &! vector field defined at U-points
      DXU,DYU             ! vector field defined at U-points

   integer (int_kind), dimension(1:nx_block*ny_block), intent(in) :: & 
      KMT
...
!-----------------------------------------------------------------------
   ...
   DIV_OUT(1+(1-1)*nx_block:1+(ny_block-1)*nx_block) = c0
   DIV_OUT(1+(1-1)*nx_block:nx_block+(1-1)*nx_block) = c0

   do j=2,ny_block
   do i=2,nx_block
      DIV_OUT((i)+(j-1)*nx_block) = p5*(UX((i  )+(j  -1)*nx_block)*DYU((i  )+(j  -1)*nx_block) +  & 
                         UX((i  )+(j-1-1)*nx_block)*DYU((i  )+(j-1-1)*nx_block) -  &
                         UX((i-1)+(j  -1)*nx_block)*DYU((i-1)+(j  -1)*nx_block) -  &
                         UX((i-1)+(j-1-1)*nx_block)*DYU((i-1)+(j-1-1)*nx_block) +  &
                         UY((i  )+(j  -1)*nx_block)*DXU((i  )+(j  -1)*nx_block) +  &
                         UY((i-1)+(j  -1)*nx_block)*DXU((i-1)+(j  -1)*nx_block) -  &
                         UY((i  )+(j-1-1)*nx_block)*DXU((i  )+(j-1-1)*nx_block) -  &
                         UY((i-1)+(j-1-1)*nx_block)*DXU((i-1)+(j-1-1)*nx_block))
   end do
   end do

   where (k > KMT) DIV_OUT = c0
