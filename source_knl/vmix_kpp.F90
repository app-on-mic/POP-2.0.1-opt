...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(:), allocatable :: & 
      ...
      zgridndr,            &! reciprocal of diff of zgrid(k)-zgrid(k+1)
      hwide,               &! layer thickness at interfaces
      hwider                ! reciprocal of hwide
...
!-----------------------------------------------------------------------
   ...
   allocate  (zgrid(0:km+1), zgridndr(0:km), hwide(0:km+1), hwider(0:km+1))
   ...
   do k=1,km
      zgrid(k) = -zt(k)
      zgridndr(k-1) = c1/(zgrid(k-1) - zgrid(k))
      hwide(k) =  dz(k)
      hwider(k) = c1/hwide(k)
   ...
   zgrid(km+1) = -zw(km)
   zgridndr(km) = c1/(zgrid(km) - zgrid(km+1))
   hwide(km+1) = eps
   hwider(km+1) = c1/hwide(km+1)
...
!-----------------------------------------------------------------------
      ...
      if (partial_bottom_cells) then
         ...
         WM = WORK*Vtc*WS*sqrt(abs(                             &
              p5*( DBLOC(:,:,kl-1)*zgridndr(kl-1) +  &
              DBLOC(:,:,kl  )*zgridndr(kl  ) ) ))
      endif
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block,3,km) :: &
...
!-----------------------------------------------------------------------
   ...
   F1 = STABLE*c5*BFSFC/((USTAR**2)**2+eps)
   ...
   do k=1,km
      ...
      if (partial_bottom_cells) then
         ...
         where (k == KN)
            ...
            GAT1(:,:,1) = VISCH * HBLTR *WMR
            DAT1(:,:,1) = -VISCP*WMR + F1*VISCH

            GAT1(:,:,2) = DIFSH * HBLTR *WSR
            DAT1(:,:,2) = -DIFSP*WSR + F1*DIFSH

            GAT1(:,:,3) = DIFTH * HBLTR *WSR
            DAT1(:,:,3) = -DIFTP*WSR + F1*DIFTH
...
!         where (k == KN)
          ...
          do i=1,nx_block
           if (k == KN(i,j)) then
            ...
            R(i,j)      = c1 - DELHAT(i,j) * hwider(k)
            DVDZUP(i,j) = (VISC(i,j,k-1) - VISC(i,j,k  ))*hwider(k)
            DVDZDN(i,j) = (VISC(i,j,k  ) - VISC(i,j,k+1))*hwider(k+1)
            VISCP(i,j)  = p5*( (c1-R(i,j))*(DVDZUP(i,j) + abs(DVDZUP(i,j))) + &
                              R(i,j) *(DVDZDN(i,j) + abs(DVDZDN(i,j))) )
            DVDZUP(i,j) = (VDC(i,j,k-1,2) - VDC(i,j,k  ,2))*hwider(k)
            DVDZDN(i,j) = (VDC(i,j,k  ,2) - VDC(i,j,k+1,2))*hwider(k+1)
            DIFSP(i,j)  = p5*( (c1-R(i,j))*(DVDZUP(i,j) + abs(DVDZUP(i,j))) + &
                              R(i,j) *(DVDZDN(i,j) + abs(DVDZDN(i,j))) )
            DVDZUP(i,j) = (VDC(i,j,k-1,1) - VDC(i,j,k  ,1))*hwider(k)
            DVDZDN(i,j) = (VDC(i,j,k  ,1) - VDC(i,j,k+1,1))*hwider(k+1)
            DIFTP(i,j)  = p5*( (c1-R(i,j))*(DVDZUP(i,j) + abs(DVDZUP(i,j))) + &
                              R(i,j) *(DVDZDN(i,j) + abs(DVDZDN(i,j))) )
            ...
            GAT1(i,j,1) = VISCH(i,j) * HBLTR(i,j) *WMR(i,j)
            DAT1(i,j,1) = -VISCP(i,j)*WMR(i,j) + F1(i,j)*VISCH(i,j)

            GAT1(i,j,2) = DIFSH(i,j) * HBLTR(i,j) *WSR(i,j)
            DAT1(i,j,2) = -DIFSP(i,j)*WSR(i,j) + F1(i,j)*DIFSH(i,j)

            GAT1(i,j,3) = DIFTH(i,j) * HBLTR(i,j) *WSR(i,j)
            DAT1(i,j,3) = -DIFTP(i,j)*WSR(i,j) + F1(i,j)*DIFTH(i,j)
           end if
          enddo
          enddo
   ...
   do k = 1,km       
      ...
      if (partial_bottom_cells) then
         if (k > 1) then
            ...
            SIGMA = (-zgrid(k) + p5*hwide(k)) * HBLTR    
         end if
      else
         SIGMA = (-zgrid(k) + p5*hwide(k)) * HBLTR    
      endif
...
!
      ...
      do j=1,ny_block
      do i=1,nx_block
      BLMC(i,j,1,k) = HBLT(i,j)*WM(i,j)*SIGMA(i,j)*(c1 + SIGMA(i,j)*((SIGMA(i,j)-c2)+ &
          (c3-c2*SIGMA(i,j))*GAT1(i,j,1)+(SIGMA(i,j)-c1)*DAT1(i,j,1))) 
      BLMC(i,j,2,k) = HBLT(i,j)*WS(i,j)*SIGMA(i,j)*(c1 + SIGMA(i,j)*((SIGMA(i,j)-c2)+ &
          (c3-c2*SIGMA(i,j))*GAT1(i,j,2)+(SIGMA(i,j)-c1)*DAT1(i,j,2)))
      BLMC(i,j,3,k) = HBLT(i,j)*WS(i,j)*SIGMA(i,j)*(c1 + SIGMA(i,j)*((SIGMA(i,j)-c2)+ &    
          (c3-c2*SIGMA(i,j))*GAT1(i,j,3)+(SIGMA(i,j)-c1)*DAT1(i,j,3)))
      end do
      end do

      GHAT(:,:,k) = (c1-STABLE)* cg*WSR
   end do
...
!-----------------------------------------------------------------------
   ...
   do j=1,ny_block
   do i=1,nx_block
   DKM1(i,j,1) = HBLT(i,j)*WM(i,j)*SIGMA(i,j)*(c1+SIGMA(i,j)*((SIGMA(i,j)-c2) &
            +(c3-c2*SIGMA(i,j))*GAT1(i,j,1)+(SIGMA(i,j)-c1)*DAT1(i,j,1)))
   DKM1(i,j,2) = HBLT(i,j)*WS(i,j)*SIGMA(i,j)*(c1+SIGMA(i,j)*((SIGMA(i,j)-c2) &
            +(c3-c2*SIGMA(i,j))*GAT1(i,j,2)+(SIGMA(i,j)-c1)*DAT1(i,j,2)))
   DKM1(i,j,3) = HBLT(i,j)*WS(i,j)*SIGMA(i,j)*(c1+SIGMA(i,j)*((SIGMA(i,j)-c2) &       
            +(c3-c2*SIGMA(i,j))*GAT1(i,j,3)+(SIGMA(i,j)-c1)*DAT1(i,j,3)))
   enddo
   enddo
...
!-----------------------------------------------------------------------
   ...
   do k=1,km-1
      ...
      if (partial_bottom_cells) then
         ...
         where (k == (KBL - 1)) &
            DELHAT = (HBLT + zgrid(k))*zgridndr(k)
      endif
      ...
      do i=1,nx_block
        if (k == (KBL(i,j) - 1)) then
         ...
         BLMC(i,j,1,k) = (c1-DELHAT(i,j))*VISC(i,j,k) +  DELHAT(i,j) *(       &
                         (c1-DELHAT(i,j))**2 *DKM1(i,j,1) + DELHAT(i,j)**2 *  &
                         (CASEA(i,j)*VISC(i,j,k)                         &
                          +(c1-CASEA(i,j))*BLMC(i,j,1,k)))

         BLMC(i,j,2,k) = (c1-DELHAT(i,j))*VDC(i,j,k,2) + DELHAT(i,j)*(        &
                         (c1-DELHAT(i,j))**2 * DKM1(i,j,2) + DELHAT(i,j)**2 * &
                         (CASEA(i,j)*VDC(i,j,k,2)                        &
                          +(c1-CASEA(i,j))*BLMC(i,j,2,k)))

         BLMC(i,j,3,k) = (c1-DELHAT(i,j))*VDC(i,j,k,1) + DELHAT(i,j) *(       &
                         (c1-DELHAT(i,j))**2 * DKM1(i,j,3) + DELHAT(i,j)**2 * &
                         (CASEA(i,j)*VDC(i,j,k,1)                        &
                         +(c1-CASEA(i,j))*BLMC(i,j,3,k)))
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block) :: &
      ...
      ZETAR,           &! d/L or sigma*hbl/L(monin-obk)
...
!-----------------------------------------------------------------------
   ...
   ZETA  = ZETAH/(USTAR*USTAR*USTAR + eps)
   do j=1,ny_block
   do i=1,nx_block
      if (ZETA(i,j) >= c0) then
         ZETAR(i,j) = c1/(c1 + c5*ZETA(i,j))
      else
         ZETAR(i,j) = c0
      endif
   end do
   ...
   if (m_or_s == 1 .or. m_or_s == 3) then
      ...
      do i=1,nx_block
         if (ZETA(i,j) >= c0) then ! stable region
            WM(i,j) = vonkar*USTAR(i,j)*ZETAR(i,j)
         else if (ZETA(i,j) >= zeta_m) then
            WM(i,j) = vonkar*USTAR(i,j)*exp(log(c1 - c16*ZETA(i,j))*p25)
         else
            WM(i,j) = vonkar*exp(log(a_m*(USTAR(i,j)**3)-c_m*ZETAH(i,j))*p33)
         endif
      end do
      end do
   endif
...
!-----------------------------------------------------------------------
   ...
   if (m_or_s == 2 .or. m_or_s == 3) then
      ...
      do i=1,nx_block
         if (ZETA(i,j) >= c0) then
            WS(i,j) = vonkar*USTAR(i,j)*ZETAR(i,j)
         else if (ZETA(i,j) >= zeta_s) then
            WS(i,j) = vonkar*USTAR(i,j)*SQRT(c1 - c16*ZETA(i,j))
         else
            WS(i,j) = vonkar*exp(log(a_s*(USTAR(i,j)**3)-c_s*ZETAH(i,j))*p33)
         endif
      end do
      end do
   endif
