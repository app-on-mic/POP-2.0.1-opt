...
! !DEFINED PARAMETERS:
   ...
   integer (int_kind), parameter, public :: & ! 
      nblock_mic_pp = 8 ,&! block# per MIC process/rank
      nproc_cpu_pn = 0 ,&! MPI process# on CPU per node
      nproc_mic_pn = 36 ,&! MPI process# on MIC per node
      nproc_pn = 36     ,&! MPI process# per node
