...
!-----------------------------------------------------------------------
   ...
   if (k < km) then
    ...
    do j = 1,ny_block
    do i = 1,nx_block
      TEMP(i,j,ks,bid) = max(-c2, TMIX(i,j,k+1,1))

      TZ(i,j,ks,1,bid) = TMIX(i,j,k,1) - TMIX(i,j,k+1,1)
      TZ(i,j,ks,2,bid) = TMIX(i,j,k,2) - TMIX(i,j,k+1,2)
      TZP(i,j,ks,bid) = TEMP(i,j,kn,bid) - TEMP(i,j,ks,bid)
      ...
      RZ(i,j) = DRDT(i,j,bid)*TZP(i,j,ks,bid) + &
           DRDS(i,j,bid)*TZ(i,j,ks,2,bid)
      RZ(i,j) = min(RZ(i,j),-eps2)
      RZM(i,j) = RZ(i,j)

      SLX(i,j,ieast ,kbt,bid) = KMASK(i,j)*RX(i,j,bid)/RZ(i,j)
      SLX(i,j,iwest ,kbt,bid) = KMASK(i,j)*RXW(i,j,bid)/RZ(i,j)
      SLY(i,j,jnorth,kbt,bid) = KMASK(i,j)*RY(i,j,bid)/RZ(i,j)
      SLY(i,j,jsouth,kbt,bid) = KMASK(i,j)*RYS(i,j,bid)/RZ(i,j)
    enddo
    enddo
