...
case $ARG_ARCH in
knl_opt ) buildcpucmd=mk_knl_opt.sh;;
* ) buildcpucmd=mk_knl_opt.sh;;
esac
...
# 1. configure block creation parameter
...
NX_BKFACTOR=3
NY_BKFACTOR=4
[ ! $((F=((BLOCK_SIZE_X/NX_BKFACTOR)*NX_BKFACTOR==BLOCK_SIZE_X))) -ne 0 ] && echo "NOT nx_bkfactor|block_size_x" && exit -1;
[ ! $((F=((BLOCK_SIZE_Y/NY_BKFACTOR)*NY_BKFACTOR==BLOCK_SIZE_Y))) -ne 0 ] && echo "NOT ny_bkfactor|block_size_y" && exit -1;
((BLOCK_SIZE_X=BLOCK_SIZE_X/NX_BKFACTOR))
((BLOCK_SIZE_Y=BLOCK_SIZE_Y/NY_BKFACTOR))
echo "tiny block_size:" $BLOCK_SIZE_X $BLOCK_SIZE_Y
sed -i '/ block_size_x = .*,/{
...
export NUM_BLOCK_PHI=$MAXBLKS
((MAXBLKS*=NX_BKFACTOR*NY_BKFACTOR))
sed -i '/max_blocks_.* = /{
    s/= [0-9]*/= '"$MAXBLKS"'/g
}' compile/domain_size.f90
