...
!         write(6,*) 'After_read_horiz_select:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
!-----------------------------------------------------------------------
!
...
!      if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'After_read_topo_select:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
   !***
...
!      if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'Before_bound_KMU:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
   call update_ghost_cells(KMU, bndy_clinic, field_loc_NEcorner, &
                                             field_type_scalar)

!      if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'After_bound_KMU:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
   !***
   ...
   if (partial_bottom_cells) then
      ...
      !$OMP END PARALLEL DO
!      if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'Before_landmasks:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
   endif
...
!      if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'After_landmasks:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
!-----------------------------------------------------------------------
!
...
!      if (my_task == nproc_cpu_pn*nnode) &
!         write(6,*) 'Before_global_sum:KMT:',my_task,sum(KMT(:,:,1:nx_bkfactor*ny_bkfactor)),sum(KMT(nghost+1:nx_block-nghost,nghost+1:ny_block-nghost,1:nx_bkfactor*ny_bkfactor))
   nocean_t = global_sum(KMT, distrb_clinic, field_loc_center)
   !ljm tuning
   !call exit_POP(sigAbort, 'Debugging global_sum')
...
!-----------------------------------------------------------------------
      ...
      !$OMP PARALLEL DO PRIVATE(this_block, i, j, ig, jg,jm1,lathalf)
      do n=1,nblocks_clinic
...
!-----------------------------------------------------------------------
   ...
   !$OMP PARALLEL DO PRIVATE(i,j,this_block,xsw,ysw,zsw,xw,yw,zw,xs,ys,zs,xc,yc,zc, &
   !$OMP                     tx,ty,tz,da)
...
!   ARRAY_TGRID(1,:) = c0
   ...
   call ugrid_to_tgrid_1d(ARRAY_TGRID, ARRAY_UGRID, &
                          AT0(:,:,iblock),ATS(:,:,iblock),ATW(:,:,iblock),ATSW(:,:,iblock))
!-----------------------------------------------------------------------
!EOC

 end subroutine ugrid_to_tgrid

!DIR$ ATTRIBUTES FORCEINLINE :: ugrid_to_tgrid_1d
 subroutine ugrid_to_tgrid_1d(ARRAY_TGRID, ARRAY_UGRID, &
                              LAT0,LATS,LATW,LATSW)
...
!-----------------------------------------------------------------------
   ...
   do i=2,nx_block
     ...
     ARRAY_TGRID((i)+(j-1)*nx_block) = LAT0 ((i)+(j-1)*nx_block)*ARRAY_UGRID((i  )+(j  -1)*nx_block) + &
                        LATS ((i)+(j-1)*nx_block)*ARRAY_UGRID((i  )+(j-1-1)*nx_block) + &
                        LATW ((i)+(j-1)*nx_block)*ARRAY_UGRID((i-1)+(j  -1)*nx_block) + &
                        LATSW((i)+(j-1)*nx_block)*ARRAY_UGRID((i-1)+(j-1-1)*nx_block)

   end do
   end do

   do i=1,nx_block
     ARRAY_TGRID((i)+(1-1)*nx_block) = c0
   end do
   do j=1,ny_block
     ARRAY_TGRID((1)+(j-1)*nx_block) = c0
   end do

!-----------------------------------------------------------------------
!EOC

 end subroutine ugrid_to_tgrid_1d
...
!   ARRAY_UGRID(nx_block,:) = c0
   ...
   call tgrid_to_ugrid_1d(ARRAY_UGRID, ARRAY_TGRID, &
                          AU0(:,:,iblock), AUN(:,:,iblock), AUE(:,:,iblock), AUNE(:,:,iblock))
...
!DIR$ ATTRIBUTES FORCEINLINE :: tgrid_to_ugrid_1d
 subroutine tgrid_to_ugrid_1d(ARRAY_UGRID, ARRAY_TGRID, &
                              LAU0, LAUN, LAUE, LAUNE)
...
!-----------------------------------------------------------------------
   ...
   do i=1,nx_block-1
     ...
     ARRAY_UGRID((i)+(j-1)*nx_block) = LAU0 ((i)+(j-1)*nx_block)*ARRAY_TGRID((i  )+(j  -1)*nx_block) + &
                        LAUN ((i)+(j-1)*nx_block)*ARRAY_TGRID((i  )+(j+1-1)*nx_block) + &
                        LAUE ((i)+(j-1)*nx_block)*ARRAY_TGRID((i+1)+(j  -1)*nx_block) + &
                        LAUNE((i)+(j-1)*nx_block)*ARRAY_TGRID((i+1)+(j+1-1)*nx_block)

   end do
   end do

   do i=1,nx_block
     ARRAY_UGRID(i+(ny_block-1)*nx_block) = c0
   end do
   do j=1,ny_block
     ARRAY_UGRID(nx_block+(j-1)*nx_block) = c0
   end do

!-----------------------------------------------------------------------
!EOC

 end subroutine tgrid_to_ugrid_1d
