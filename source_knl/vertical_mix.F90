...
!      hfac_t
   ...
   integer (int_kind) :: &
      i1,                &! dummpy loop index
      ie, ilen,          &! remainder loop count
      k_min,             &! aggregate min
      k_max               ! aggregate max

   real (r8),  dimension(VLEN) :: &
      A,B,C,D             ! various temporaries

   real (r8), dimension(VLEN,km) :: & 
      E,F,               &! various work arrays
      HFAC_T
...
!      H1 = hfac_t(1) + PSFC/(grav*c2dtt(1))
      H1 = HFAC_T(1,1) + PSFC/(grav*c2dtt(1))
   else
!      H1 = hfac_t(1)
      H1 = HFAC_T(1,1)
   endif
...
!-----------------------------------------------------------------------
   ...
   do n = nfirst,nlast
      ...
      do i=this_block%ib,this_block%ie-VLEN+1,VLEN

         k_min = minval(KMT(i:i+VLEN-1,j,bid))
         k_max = maxval(KMT(i:i+VLEN-1,j,bid))
         !*** perform tridiagonal solve in the vertical for every
         !*** horizontal grid point

        !DIR$ VECTOR
        do i1=1,VLEN
         A(i1) = afac_t(1)*VDC(i+i1-1,j,1,mt2,bid)
         D(i1) = c1/(H1(i+i1-1,j) + A(i1))
         E(i1,1) = A(i1)*D(i1)
         B(i1) = H1(i+i1-1,j)*E(i1,1)
         F(i1,1) = HFAC_T(i1,1)*TNEW(i+i1-1,j,1,n)*D(i1)
         F(i1,k_min+1:km) = c0
        end do
        ...
        do k=2,k_max
            !DIR$ VECTOR
          do i1=1,VLEN
          if (k <= KMT(i+i1-1,j,bid)) then
            C(i1) = A(i1)

               A(i1) = aidif*VDC(i+i1-1,j,k,mt2,bid)/ &
                   (p5*(DZT(i+i1-1,j,k  ,bid) + &
                        DZT(i+i1-1,j,k+1,bid)))
               HFAC_T(i1,k) = DZT(i+i1-1,j,k,bid)/c2dtt(k)

            if (k == KMT(i+i1-1,j,bid)) then
               D(i1) = c1/(HFAC_T(i1,k)+B(i1))
            else
               D(i1) = c1/(HFAC_T(i1,k)+A(i1)+B(i1))
            endif

            E(i1,k) = A(i1)*D(i1)
            B(i1) = (HFAC_T(i1,k) + B(i1))*E(i1,k)

            F(i1,k) = (HFAC_T(i1,k)*TNEW(i+i1-1,j,k,n) + C(i1)*F(i1,k-1))*D(i1)
          endif
         end do
         end do ! k=2,k_max
        ...
        do k=2,k_max
          ...
          if (k <= KMT(i+i1-1,j,bid)) then
            C(i1) = A(i1)
               ...
               A(i1) = afac_t(k)*VDC(i+i1-1,j,k,mt2,bid)
         ...
         do i1=1,VLEN
         do k=KMT(i+i1-1,j,bid)-1,k_min,-1
            F(i1,k) = F(i1,k) + E(i1,k)*F(i1,k+1)
         end do
         end do
...
!-----------------------------------------------------------------------
         ...
         do i1=1,VLEN
         do k = 1,km
            TNEW(i+i1-1,j,k,n) = TOLD(i+i1-1,j,k,n) + F(i1,k)
         end do
         end do
      ...
      ! remainder i loop
      ie = this_block%ie
      ilen = this_block%ie - i + 1
      if (i <= ie) then
         k_min = minval(KMT(i:ie,j,bid))
         k_max = maxval(KMT(i:ie,j,bid))
         !*** perform tridiagonal solve in the vertical for every
         !*** horizontal grid point
...
!      do i=this_block%ib,this_block%ie
      do i=this_block%ib,this_block%ie-VLEN+1,VLEN
        ...
        do i1=1,VLEN
         ...
         E(i1,1) = A(i1)*D(i1)
         B(i1) = H1(i+i1-1,j)*E(i1,1)
         F(i1,1) = HFAC_T(i1,1)*RHS(i+i1-1,j,n)*D(i1)
         F(i1,k_min+1:km) = c0
        end do
...
!-----------------------------------------------------------------------
         ...
         do i1=1,VLEN
         do k = 1,km
            TNEW(i+i1-1,j,k,n) = TNEW(i+i1-1,j,k,n) + F(i1,k)
         end do
         end do
...
!   do i=this_block%ib,this_block%ie
   do i=this_block%ib,this_block%ie-VLEN+1,VLEN
      ...
      k_min = minval(KMU(i:i+VLEN-1,j,bid))
      k_max = maxval(KMU(i:i+VLEN-1,j,bid))
      !*** solve tridiagonal system at each grid point

      !DIR$ VECTOR
      do i1=1,VLEN
       A(i1) = afac_u(1)*VVC(i+i1-1,j,1,bid)
       D(i1) = c1/(HFAC_U(i1,1) + A(i1))
       E(i1,1) = A(i1)*D(i1)
       B(i1) = HFAC_U(i1,1)*E(i1,1)
       F1(i1,1) = HFAC_U(i1,1)*UNEW(i+i1-1,j,1)*D(i1)
       F2(i1,1) = HFAC_U(i1,1)*VNEW(i+i1-1,j,1)*D(i1)
       F1(i1,k_min+1:km) = c0
       F2(i1,k_min+1:km) = c0
      end do
      ...
      do k=2,k_max
          !DIR$ VECTOR
        do i1=1,VLEN
        if (k <= KMU(i+i1-1,j,bid)) then
          C(i1) = A(i1)

             HFAC_U(i1,k) = DZU(i+i1-1,j,k,bid)/c2dtu
             A(i1) = aidif*VVC(i+i1-1,j,k,bid)/ &
                 (p5*(DZU(i+i1-1,j,k  ,bid) + &
                      DZU(i+i1-1,j,k+1,bid)))

          if (k == KMU(i+i1-1,j,bid)) then
             D(i1) = c1/(HFAC_U(i1,k)+B(i1))
          else
             D(i1) = c1/(HFAC_U(i1,k)+A(i1)+B(i1))
          endif

          E(i1,k) = A(i1)*D(i1)
          B(i1) = (HFAC_U(i1,k) + B(i1))*E(i1,k)

          F1(i1,k) = (HFAC_U(i1,K)*UNEW(i+i1-1,j,k) + C(i1)*F1(i1,K-1))*D(i1)
          F2(i1,k) = (HFAC_U(i1,K)*VNEW(i+i1-1,j,k) + C(i1)*F2(i1,K-1))*D(i1)
        endif
       end do
       end do ! k=2,k_max
      ...
      do k=2,k_max
        ...
        if (k <= KMU(i+i1-1,j,bid)) then
          C(i1) = A(i1)
             ...
             A(i1) = afac_u(k)*VVC(i+i1-1,j,k,bid)
       ...
       do i1=1,VLEN
       do k=KMU(i+i1-1,j,bid)-1,k_min,-1
          F1(i1,k) = F1(i1,k) + E(i1,k)*F1(i1,k+1)
          F2(i1,k) = F2(i1,k) + E(i1,k)*F2(i1,k+1)
       end do
       end do
       ...
       do i1=1,VLEN
       do k = 1,km
          UNEW(i+i1-1,j,k) = F1(i1,k)
          VNEW(i+i1-1,j,k) = F2(i1,k)
       end do
       end do
   ...
   ilen = this_block%ie - i + 1
   if (i <= ie) then
      k_min = minval(KMU(i:ie,j,bid))
      k_max = maxval(KMU(i:ie,j,bid))
      !*** solve tridiagonal system at each grid point
