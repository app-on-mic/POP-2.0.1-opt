...
!      end do
      ...
      call hdiffu_del4_1d_sec1(D2UK,D2VK,this_block, &
                 UMIXK,VMIXK,      &
                 CC,               &
                 DUN(:,:,bid),DUS(:,:,bid),DUE(:,:,bid),DUW(:,:,bid),  &
                 DMC(:,:,bid),DMN(:,:,bid),DMS(:,:,bid),DME(:,:,bid),  &
                 DMW(:,:,bid))
...
!      end do
      ...
      call hdiffu_del4_1d_sec2(HDUK,HDVK,this_block, &
                 D2UK,D2VK,        &
                 CC,               &
                 DUN(:,:,bid),DUS(:,:,bid),DUE(:,:,bid),DUW(:,:,bid),  &
                 DMC(:,:,bid),DMN(:,:,bid),DMS(:,:,bid),DME(:,:,bid),  &
...
!DIR$ ATTRIBUTES FORCEINLINE :: hdiffu_del4_1d_sec1
 subroutine hdiffu_del4_1d_sec1(D2UK,D2VK,this_block, &
                 UMIXK,VMIXK,        &
                 CC,               &
                 DUN,DUS,DUE,DUW,  &
                 DMC,DMN,DMS,DME,  &
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      do i=this_block%ib-1,this_block%ie+1
         ...
         D2UK((i)+(j-1)*nx_block)  =(CC ((i)+(j-1)*nx_block)*UMIXK((i  )+(j  -1)*nx_block) +        &
                      DUN((i)+(j-1)*nx_block)*UMIXK((i  )+(j+1-1)*nx_block) +        &
                      DUS((i)+(j-1)*nx_block)*UMIXK((i  )+(j-1-1)*nx_block) +        &
                      DUE((i)+(j-1)*nx_block)*UMIXK((i+1)+(j  -1)*nx_block) +        &
                      DUW((i)+(j-1)*nx_block)*UMIXK((i-1)+(j  -1)*nx_block))+        &
                     (DMC((i)+(j-1)*nx_block)*VMIXK((i  )+(j  -1)*nx_block) +        &
                      DMN((i)+(j-1)*nx_block)*VMIXK((i  )+(j+1-1)*nx_block) +        &
                      DMS((i)+(j-1)*nx_block)*VMIXK((i  )+(j-1-1)*nx_block) +        &
                      DME((i)+(j-1)*nx_block)*VMIXK((i+1)+(j  -1)*nx_block) +        &
                      DMW((i)+(j-1)*nx_block)*VMIXK((i-1)+(j  -1)*nx_block))
         D2VK((i)+(j-1)*nx_block)  =(CC ((i)+(j-1)*nx_block)*VMIXK((i  )+(j  -1)*nx_block) +        &
                      DUN((i)+(j-1)*nx_block)*VMIXK((i  )+(j+1-1)*nx_block) +        &
                      DUS((i)+(j-1)*nx_block)*VMIXK((i  )+(j-1-1)*nx_block) +        &
                      DUE((i)+(j-1)*nx_block)*VMIXK((i+1)+(j  -1)*nx_block) +        &
                      DUW((i)+(j-1)*nx_block)*VMIXK((i-1)+(j  -1)*nx_block))-        &
                     (DMC((i)+(j-1)*nx_block)*UMIXK((i  )+(j  -1)*nx_block) +        &
                      DMN((i)+(j-1)*nx_block)*UMIXK((i  )+(j+1-1)*nx_block) +        &
                      DMS((i)+(j-1)*nx_block)*UMIXK((i  )+(j-1-1)*nx_block) +        &
                      DME((i)+(j-1)*nx_block)*UMIXK((i+1)+(j  -1)*nx_block) +        &
                      DMW((i)+(j-1)*nx_block)*UMIXK((i-1)+(j  -1)*nx_block))
      end do
      end do
...
!EOC
 ...
 end subroutine hdiffu_del4_1d_sec1

!DIR$ ATTRIBUTES FORCEINLINE :: hdiffu_del4_1d_sec2
 subroutine hdiffu_del4_1d_sec2(HDUK,HDVK,this_block, &
                 D2UK,D2VK,        &
                 CC,               &
                 DUN,DUS,DUE,DUW,  &
                 DMC,DMN,DMS,DME,  &
...
! !INPUT PARAMETERS:
   ...
   real (r8), dimension(1:nx_block*ny_block), intent(out) :: &
      HDUK,HDVK          ! intermediate Del**2 results
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      do j=this_block%jb,this_block%je
      do i=this_block%ib,this_block%ie
         HDUK((i)+(j-1)*nx_block)  = am*((CC ((i)+(j    -1)*nx_block)*D2UK((i  )+(j  -1)*nx_block) +        &
                           DUN((i)+(j-1)*nx_block)*D2UK((i  )+(j+1-1)*nx_block) +        &
                           DUS((i)+(j-1)*nx_block)*D2UK((i  )+(j-1-1)*nx_block) +        &
                           DUE((i)+(j-1)*nx_block)*D2UK((i+1)+(j  -1)*nx_block) +        &
                           DUW((i)+(j-1)*nx_block)*D2UK((i-1)+(j  -1)*nx_block))+        &
                          (DMC((i)+(j-1)*nx_block)*D2VK((i  )+(j  -1)*nx_block) +        &
                           DMN((i)+(j-1)*nx_block)*D2VK((i  )+(j+1-1)*nx_block) +        &
                           DMS((i)+(j-1)*nx_block)*D2VK((i  )+(j-1-1)*nx_block) +        &
                           DME((i)+(j-1)*nx_block)*D2VK((i+1)+(j  -1)*nx_block) +        &
                           DMW((i)+(j-1)*nx_block)*D2VK((i-1)+(j  -1)*nx_block)))
         HDVK((i)+(j-1)*nx_block)  = am*((CC ((i)+(j    -1)*nx_block)*D2VK((i  )+(j  -1)*nx_block) +        &
                           DUN((i)+(j-1)*nx_block)*D2VK((i  )+(j+1-1)*nx_block) +        &
                           DUS((i)+(j-1)*nx_block)*D2VK((i  )+(j-1-1)*nx_block) +        &
                           DUE((i)+(j-1)*nx_block)*D2VK((i+1)+(j  -1)*nx_block) +        &
                           DUW((i)+(j-1)*nx_block)*D2VK((i-1)+(j  -1)*nx_block))-        &
                          (DMC((i)+(j-1)*nx_block)*D2UK((i  )+(j  -1)*nx_block) +        &
                           DMN((i)+(j-1)*nx_block)*D2UK((i  )+(j+1-1)*nx_block) +        &
                           DMS((i)+(j-1)*nx_block)*D2UK((i  )+(j-1-1)*nx_block) +        &
                           DME((i)+(j-1)*nx_block)*D2UK((i+1)+(j  -1)*nx_block) +        &
                           DMW((i)+(j-1)*nx_block)*D2UK((i-1)+(j  -1)*nx_block)))
      end do
      end do
...
!EOC
 ...
 end subroutine hdiffu_del4_1d_sec2
